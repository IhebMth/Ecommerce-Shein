...
function initCountdown() {
  ...
  function _updateLabels() {
    /* Guard: timer elements may not exist if SALE_CONFIG.active is false */
    const labelEl = document.getElementById('sc-label');
    if (!labelEl) return;
    const u = SALE_CONFIG.units[_lang()] || SALE_CONFIG.units.en;
    labelEl.textContent = _t(SALE_CONFIG.label);
    const dn = document.getElementById('sc-d-name'); if (dn) dn.textContent = u.days;
    const hn = document.getElementById('sc-h-name'); if (hn) hn.textContent = u.hours;
    const mn = document.getElementById('sc-m-name'); if (mn) mn.textContent = u.mins;
    const sn = document.getElementById('sc-s-name'); if (sn) sn.textContent = u.secs;
  }
